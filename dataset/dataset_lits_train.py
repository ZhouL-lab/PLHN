...
class Lits_DataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pre = self.normalization(self.load(os.path.join(self.root,file,'P0.nii.gz'))).astype(np.float32)
        pos = self.normalization(self.load(os.path.join(self.root,file,'P1.nii.gz'))).astype(np.float32)
        sub = pos - pre
