...
class ux_block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        self.norm =  nn.InstanceNorm3d(dim)#LayerNorm(dim, eps=1e-6)
...
class uxnet_conv(nn.Module):
    ...
    def __init__(self, in_chans=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384],
        ...
        for i_layer in range(4):
            layer = nn.InstanceNorm3d(dims[i_layer])#norm_layer(dims[i_layer])
...
if __name__=='__main__':
    # 96,160,160 48,192,192
    model  = UXNET(in_chans=1,out_chans=14)
    model = model.cuda()
    img = torch.randn((4, 1, 48,192,192)).cuda()
    img = model(img)
    print(img.size())