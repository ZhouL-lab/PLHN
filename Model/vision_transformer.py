...
class SwinUnet3D(nn.Module):
    def __init__(self,  num_classes=2, in_chans=2, zero_head=False, vis=False):
        ...
        self.swin_unet3D = SwinTransformer3D(patch_size=(2, 4, 4),
                                             num_classes=self.num_classes,
                                             in_chans=self.in_chans,
                                             embed_dim=48,
                                             depths=[2, 2, 6, 2],
    ...
    def load_from(self, ):
        pretrained_path = '/media/usst/data/resunet/checkpoints_RUENT_8/tmi/lw_swin_unet/model/swin_tiny_patch4_window7_224.pth'
        if pretrained_path is not None:
            print("pretrained_path:{}".format(pretrained_path))
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            pretrained_dict = torch.load(pretrained_path, map_location=device)
