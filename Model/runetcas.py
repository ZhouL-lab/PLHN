...
class RUNet_cas(nn.Module):
    def __init__(self, in_dim, out_dim, num_filters):
        super(RUNet_cas, self).__init__()

        self.seg_net = RUNet(in_dim, out_dim, num_filters)
        self.syn_net = RUNet(in_dim-1, out_dim, num_filters)

    def forward(self, pos,pre,sub):
        seg = self.seg_net(torch.cat([pos,sub],1))
        syn = self.syn_net(seg*pre)
