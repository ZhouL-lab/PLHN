...
class PR_AFFINITY_NET(nn.Module):
 ...
 # For network1
    ...
    ###################
        self.Addweight1 = nn.Parameter(torch.ones(1)*0.1)
        self.Addweight2 = nn.Parameter(torch.ones(1)*0.1)
        self.Addconcatweight = nn.Parameter(torch.ones(1))
        self.Addmultipweight = nn.Parameter(torch.ones(1)*0.1)
        self.fuselayer = nn.Sequential(
            conv_block_3d(self.out_dim*2, self.out_dim, activation),
            conv_block_3d(self.out_dim, self.out_dim, act1)
        )
    ...
    ####Network for learning features for ensemble learning ##
        ...
        finalseg = torch.sigmoid(self.Addweight1)*pred_att+(1-torch.sigmoid(self.Addweight1))*out #+(torch.sigmoid(self.Addmultipweight))*(out*pred_att)
        #finalseg = torch.sigmoid(finalseg)
