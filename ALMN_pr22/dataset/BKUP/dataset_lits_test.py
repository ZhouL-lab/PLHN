...
def Test_all_Datasets(dataset_path,size, test_folder = 0):
    f = open(os.path.join(dataset_path,'data_folder', 'test'+str(test_folder)+'.txt'))
    data_list = f.read().splitlines()
    print("The number of test samples is: ", len(data_list))
    for file in data_list:
        #file = str(int(file))
        print("\nStart Evaluate: ", file)
        pre = normalization(load(os.path.join(dataset_path, 'plain/', file + '.nii.gz'))).astype(np.float32)
        pos = normalization(load(os.path.join(dataset_path, 'dyn1/', file + '.nii.gz'))).astype(np.float32)
        sub = pos - pre
        #print(sub.shape)
        gt = load(os.path.join(dataset_path, 'label_fine/', file+ '.nii.gz')).astype(np.int16)
        breast_mask = load(os.path.join(dataset_path, 'Breast_mask/',file+'.nii.gz')).astype(np.int16)
