...
class PR_AFFINITY_NET(nn.Module):
 ...
 # For network1
    ...
    ##The obtain the subnetwork level affinity matrics
        ...
        pred_f=mask1_ref*0.5+mask2_ref*0.5 ##两个网络融合的分割结果。

    ####Network for learning features for ensemble learning ##
        x=outf ## To fuse the output.
        x1=self.down_block(torch.cat([up_5,img,x],1))
        Fe3=self.up_block(x1) ##TO learn the features for affinity.
        aff3=self._affonly(Fe3) ##This the fused affinity
        aff3org=aff3

        AFFC=0
        if AFFC==1:
            aff3=torch.squeeze(aff3,1)
            aff1=torch.squeeze(aff1,1)
            aff2=torch.squeeze(aff2,1)

            uaf3=self.Gru3dAF1(aff1,aff2) ##To fuse the two.
            #uaf3=self.Gru3dAF2(aff2,uaf3)
            uaf3=torch.unsqueeze(uaf3,1)
            s1=(1e-8 + uaf3.sum(2))
            uaf3=uaf3/s1
            affave=uaf3*0.4+0.6*aff3org #0.4, 0.6 for 78.0
        else:
            affave=aff3

        res=self._affseg(affave,pred_f)
        finalseg=torch.where(res>1,torch.ones_like(res),res)
        #finalseg = torch.where(finalseg<0, torch.ones_like(finalseg)*0, finalseg)

        pred_f = torch.where(pred_f > 1, torch.ones_like(pred_f), pred_f)
        #pred_f = torch.where(pred_f < 0, torch.ones_like(pred_f) * 0, pred_f)

        return finalseg, pred_f, pred_att, out  ##最后的分割结果，和中间融合的分割结果