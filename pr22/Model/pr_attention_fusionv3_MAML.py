 ...
 # For network1
    ...
    ########For network2
        ...
        self.fuselayer = nn.Sequential(
            conv_block_2_3d(self.num_filters+self.achannel, self.num_filters+self.achannel, activation),

        )
        self.fuseattention1 = nn.Sequential(
            conv_block_2_3d(self.num_filters+self.achannel, self.achannel, activation),
            conv_block_3d(self.achannel, self.achannel, activation),
            conv_block_3d(self.achannel, self.out_dim, act1)
        )
        self.fuseattention2 = nn.Sequential(
            conv_block_2_3d(self.num_filters+self.achannel, self.num_filters, activation),
            conv_block_3d(self.num_filters, self.num_filters, activation),
            conv_block_3d(self.num_filters, self.out_dim, act1)
        )
        self.Addweight1 = nn.Parameter(torch.ones(1))
        self.Addweight2 = nn.Parameter(torch.ones(1))
    ...
    ####Network for learning features for ensemble learning ##
        ##融合pred_att, out; 代替(pred_att+out)/2
        fuserefine = self.upsample6(refine)
        fuseup_5 = up_5
        fuselayer =  self.fuselayer(torch.cat([fuserefine,fuseup_5],dim=1))
        fusePreattention = self.fuseattention1(fuselayer)*pred_att ##每部分选择一部分概率。
        fuseOutattention = self.fuseattention2(fuselayer)*out
        finalseg = fusePreattention+fuseOutattention
